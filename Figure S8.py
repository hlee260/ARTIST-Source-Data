...
for j in (L_0_list):
    ...
    sim_unbound_dART_list_upper.append(calculate_unbound_dART(L_0_list[j], 40) # Upper Kd
    sim_unbound_dART_list_lower.append(calculate_unbound_dART(L_0_list[j], 1.6)) # Lower Kd
...
# Change to below for lower or upper bound Kd
plt.plot(L_0_list, sim_Reporter_reacted_list_upper, color='orange', linestyle = '--', label = 'Simulation', linewidth=lw) 
plt.plot(L_0_list, sim_Reporter_reacted_list_lower, color='orange', linestyle = '--', label = 'Simulation', linewidth=lw) 
